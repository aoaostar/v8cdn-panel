<template>
  <div class="layout">
    ...
    <el-container>
      <el-main class="panel">
        <el-card v-loading="loading" class="card box-card" :body-style="{}">
...
import { fetchGet, fetchPut } from "@utils/requests";
export default {
  name: "Edit",
  components: {
    Navbar,
  ...
  created() {
    this.get_dnsrecord(this.zone_id, this.record_id);
  },
  methods: {
    submit() {
    ...
    get_dnsrecord(zone_id, record_id) {
      fetchGet("/api/dnsrecord", {
        zone_id,
        record_id,
      }).then((res) => {
        if (res.status === "ok") {
